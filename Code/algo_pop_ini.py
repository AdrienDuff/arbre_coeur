...
def algoPrimAlea1(n_num, n, tab_sommets, tab_cout, n_proche):
	"""docstring for algoPrimAlea1 :
		...
		tab_cout : tableau de cout des sommets"""
	...
	for k in range(1,n):
		...
		#for i in [l for l in tab_indice if (b[l] == False and (h[l] == -1 or getcout(tab_cout,tab_sommets[l],tab_sommets[i_hder]) < R))]:
		for i in [l for l in tab_indice if b[l] == False]:
			if h[i] == -1 or getcout(tab_cout,tab_sommets[i],tab_sommets[i_hder]) < getcout(tab_cout,tab_sommets[i],tab_sommets[h[i]]): #Si on avait pas encore vu ce sommet au moins une fois, il faut rentrer dans cette condition.
				h[i] = i_hder
				c[i] = getcout(tab_cout,tab_sommets[i],tab_sommets[h[i]])
				pq.add_task(i,c[i])
	...
	return TreeStruct(n, 1, n_num, tree)


def algoPrimAlea2(n_num, n, tab_sommets, tab_cout, n_proche):
...
def algoPrim(n_num, n, tab_sommets, tab_cout):
	"""docstring for algoPrimAlea2 :
		n_num : Numéro de l'arbre généré dans la génération
		n : Nombre de sommets de l'arbre
		tab_sommets : tableau des sommets que l'on prend
